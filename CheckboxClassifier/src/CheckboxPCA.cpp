...
int CheckboxPCA::getClassificationIdx(const string& filepath) {
	/*int nameIdx = filepath.find_last_of("\\");
	string filename = filepath.substr(nameIdx + 1, filepath.size() - nameIdx);*/
	string classification = filepath.substr(0, filepath.find_first_of("/"));
...
void CheckboxPCA::PCA_set_push_back(Mat& PCA_set, const Mat& img) {
	Mat PCA_set_row;
...
void CheckboxPCA::PCA_set_add(Mat& PCA_set, int x, int y, vector<int>& trainingBubbleValues, const string& filename, bool flipExamples) {
	...
	Rect r(x, y, exampleSize.width, exampleSize.height);
	example = example(r);
...
bool CheckboxPCA::train_classifier(const vector<string>& examplePaths, Size myExampleSize, int x, int y, int eigenvalues, bool flipExamples) {
	...
	for(size_t i = 0; i < examplePaths.size(); i++) {
		PCA_set_add(PCA_set, x, y, trainingBubbleValues, examplePaths[i], flipExamples);
	}
...
inline double CheckboxPCA::rate_item(const Mat& det_img_gray, const Point& item_location) const {
...
Point CheckboxPCA::align_item(const Mat& det_img_gray, const Point& seed_location, double alignment_radius) const {
	int iterations = 10;
	#define CHECKED 123
...
string CheckboxPCA::classify_item(const Mat& det_img_gray, const Point& item_location) const {
...
void CheckboxPCA::save(const string& outputPath) const throw(cv::Exception){
...
void CheckboxPCA::load(const string& inputPath) throw(cv::Exception){
