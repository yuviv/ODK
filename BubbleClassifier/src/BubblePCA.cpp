...
int BubblePCA::getClassificationIdx(const string& filepath) {
...
void BubblePCA::PCA_set_push_back(Mat& PCA_set, const Mat& img) {
	Mat PCA_set_row;
...
void BubblePCA::PCA_set_add(Mat& PCA_set, int x, int y, vector<int>& trainingBubbleValues, const string& filename, bool flipExamples) {
...
bool BubblePCA::train_classifier(const vector<string>& examplePaths, Size myExampleSize, int x, int y, int eigenvalues, bool flipExamples) {
...
inline double BubblePCA::rate_item(const Mat& det_img_gray, const Point& item_location) const {
...
Point BubblePCA::align_item(const Mat& det_img_gray, const Point& seed_location, double alignment_radius) const {
	int iterations = 10;
	#define CHECKED 123
...
string BubblePCA::classify_item(const Mat& det_img_gray, const Point& item_location) const {
...
void BubblePCA::save(const string& outputPath) const throw(cv::Exception){
...
void BubblePCA::load(const string& inputPath) throw(cv::Exception){
